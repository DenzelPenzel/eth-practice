...
const json = getContract(
  require(`${process.env.USER_BUILD_DIR}/uniswap-v3-001.json`),
  "uniswap-v3-001.sol",
  "UniswapV3SingleHopSwap"
);

const AMOUNT_IN = 10n ** 18n;
const AMOUNT_OUT = 5n * 10n ** 18n;
const AMOUNT_IN_MAX = 10n ** 18n;

describe("UniswapV3SingleHopSwap", () => {
  let contract;
  let accounts = [];
  let user;
  let weth;
  ...
  it("swapExactInputSingleHop", async () => {
    log(`Swapping ${format(AMOUNT_IN)} WETH to DAI`);

    await contract.connect(user).swapExactInputSingleHop(AMOUNT_IN, 1);
  ...
  it("swapExactOutputSingleHop", async () => {
    log(
    ...
    await contract
      .connect(user)
      .swapExactOutputSingleHop(AMOUNT_OUT, AMOUNT_IN_MAX);
